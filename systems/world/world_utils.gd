...
static func world_to_cell(world_position: Vector2) -> Vector2i:
	...
	return Vector2i(
		int(floor(world_position.x / WorldConstants.CELL_SIZE.x)),
		int(floor(world_position.y / WorldConstants.CELL_SIZE.y))
	)
...
static func cell_to_world(cell_position: Vector2i) -> Vector2:
	...
	return Vector2(
		cell_position.x * WorldConstants.CELL_SIZE.x,
		cell_position.y * WorldConstants.CELL_SIZE.y
	)
...
static func world_to_chunk(world_position: Vector2) -> Vector2i:
	...
	return Vector2i(
		int(floor(float(cell_position.x) / WorldConstants.CHUNK_SIZE_CELLS.x)),
		int(floor(float(cell_position.y) / WorldConstants.CHUNK_SIZE_CELLS.y))
	)
...
static func chunk_to_world(chunk_position: Vector2i) -> Vector2:
	...
	var chunk_cell_origin := Vector2i(
		chunk_position.x * WorldConstants.CHUNK_SIZE_CELLS.x,
		chunk_position.y * WorldConstants.CHUNK_SIZE_CELLS.y
	)
